...
public class RedTeleOpNewerest extends OpMode {
    ...
    // ═══ FLIPPED FOR RED: X mirrored (144 - 60.5 = 83.5), Y mirrored (144 - 100 = 44), heading +180° ═══
    private final Pose startPose = new Pose(83.5, 44, Math.toRadians(143 + 180));
    private final Pose redTarget = new Pose(83.5, 44, Math.toRadians(143 + 180));
    ...
    public void loop() {
        ...
        if (waitingForContinue) {
            ...
            multiTelemetry.addLine("Arrived at RED Target — Press B to continue");
            multiTelemetry.update();
            return;
        }
        ...
        if (navigating) {
            ...
            // ═══ FLIPPED JOYSTICK MAPPING (180° rotation) ═══
            double rotatedX = gamepad1.left_stick_y;  // Inverted forward/back → strafe
            double rotatedY = gamepad1.left_stick_x;   // Inverted left/right → drive
            double rotation = gamepad1.right_stick_x;  // Inverted rotation
    ...
    private void updateLEDs() {
        ...
        // ✅ SAME LED LOGIC AS BLUE VERSION
